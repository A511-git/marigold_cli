...
try:
    ...
    class _TritonStub(types.ModuleType):
        ...
        def __call__(self, *args, **kwargs):
            if args and callable(args[0]):
                return args[0]
            return _TritonStub("stub")
        def __getitem__(self, item):
            return _TritonStub(str(item))
    ...
    triton.testing = _TritonStub("triton.testing")
    triton.language = _TritonStub("triton.language")
    triton.runtime = _TritonStub("triton.runtime")

    class _DummyAutotuner:
        def __init__(self, fn, *args, **kwargs):
            self.fn = fn
        def __call__(self, *args, **kwargs):
            return self.fn(*args, **kwargs)

    triton.runtime.Autotuner = _DummyAutotuner
    triton.runtime.Config = lambda *args, **kwargs: None
    triton.Config = lambda *args, **kwargs: None
    triton.jit = lambda fn: fn
    triton.cdiv = lambda a, b: (a + b - 1) // b
    triton.testing.do_bench = lambda *args, **kwargs: 0.0

    sys.modules.setdefault("triton", triton)
    sys.modules.setdefault("triton.testing", triton.testing)
    sys.modules.setdefault("triton.language", triton.language)
    sys.modules.setdefault("triton.runtime", triton.runtime)
    HAS_TRITON = False
...
def triton_autotune(configs, key, prune_configs_by=None, reset_to_zero=None, restore_value=None, pre_hook=None, post_hook=None,
    ...
    def decorator(fn):
        ...
        return TritonPersistentCacheAutotuner(
            fn, getattr(fn, 'arg_names', []), configs, key, reset_to_zero, restore_value, pre_hook=pre_hook,
